...
def compare_images(indexpairs,imagematrix,pca_list,Target,Best):
    ...
    imagematrix_nextgen = imagematrix[newindices]
    pca_list_nextgen = pca_list[newindices]
    return imagematrix_nextgen,pca_list_nextgen,maxcorr,avgcorr,Best
...
while not success:
    ...
    newmatrix,newpca_list,maxcorr,avgcorr,Best = compare_images(indexpairs,newmatrix,newpca_list,Target,Best)
...
for i in range(100):
    for j in range(100):
        if i == j:
            continue
        homogeneity.append(pearsonr(newmatrix[i],newmatrix[j])[0])


# In[50]:

print np.mean(homogeneity)
